...
daily_col = ['open', 'high', 'low', 'close', 'pre_close', 'change', 'pct_chg', 'vol', 'amount']
daily_dict = {}
for v in daily_col:
    daily_dict[v] = tl.table2mat(daily,v,'trade_date')


daily_basic_col = ['close', 'turnover_rate', 'turnover_rate_f',  'pe_ttm', 
...
for v in daily_dict.keys():
    daily_dict[v].to_pickle(matrix_path + v + '.pkl')

for v in daily_basic_dict.keys():
    daily_basic_dict[v].to_pickle(matrix_path + v + '.pkl')
