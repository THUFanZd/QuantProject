...
#%% 更新交易日
data_start = 2010
today = tl.get_trade_date()
endDate = int(tl.get_trade_date()[:4])
quarters = ['0331','0630','0930','1231']
fin_path_raw = config['paths']['fin']

basic_col = 'ts_code,ann_date,f_ann_date,end_date,report_type,comp_type,update_flag,'

fin_col = 'money_cap,trad_asset,notes_receiv,oth_cur_assets,pur_resale_fa,nca_within_1y,total_cur_assets,'
fin_col = fin_col + 'accounts_receiv,prepayment,oth_receiv,inventories,amor_exp,'
fin_col = fin_col + 'fa_avail_for_sale,htm_invest,lt_eqt_invest,invest_real_estate,time_deposits,'
fin_col = fin_col + 'fix_assets,cip,const_materials,fixed_assets_disp,produc_bio_assets,intan_assets,r_and_d,goodwill,lt_amor_exp,total_nca,total_assets,'
fin_col = fin_col + 'oth_nca,contract_assets,deriv_assets,loanto_oth_bank_fi,'
fin_col = fin_col + 'lt_borr,st_borr,trading_fl,notes_payable,acct_payable,adv_receipts,st_bonds_payable,non_cur_liab_due_1y,oth_cur_liab,total_cur_liab,bond_payable,lt_payable,estimated_liab,total_liab'
balance_col = basic_col+ fin_col


fin_col = 'basic_eps,revenue,total_cogs,oper_cost,int_exp,comm_exp,biz_tax_surchg,sell_exp,admin_exp,fin_exp,'
fin_col = fin_col + 'total_profit,income_tax,ebit,ebitda,rd_exp'
income_col = basic_col + fin_col

fin_col = 'c_fr_sale_sg,c_inf_fr_operate_a,st_cash_out_act,n_cashflow_act,'
fin_col = fin_col + 'stot_inflows_inv_act,stot_out_inv_act,n_cashflow_inv_act,'
fin_col = fin_col + 'stot_cash_in_fnc_act,stot_cashout_fnc_act,n_cash_flows_fnc_act,'
cash_col = basic_col + fin_col


# indi = 'ts_code,ann_date,end_date,'
# indi = indi + 'q_saleexp_to_gr,q_gc_to_gr,q_roe,q_dt_roe,q_npta,q_ocf_to_sales,q_sales_yoy,q_op_qoq'

while data_start <= endDate:
    for q in quarters:
        q = str(data_start) + q
        print(q)
        balance_ = pro.balancesheet_vip(period=q,fields=balance_col)
        income_ = pro.income_vip(period=q,fields=income_col)
        cash_ = pro.cashflow_vip(period=q,fields=cash_col)
        balance_ = balance_[balance_['ts_code'].str[-2:].isin(['SZ', 'SH'])]
        income_ = income_[income_['ts_code'].str[-2:].isin(['SZ', 'SH'])]
        cash_ = cash_[cash_['ts_code'].str[-2:].isin(['SZ', 'SH'])]
        balance_ = balance_[~balance_['ts_code'].str[0].isin(['A'])]
        income_ = income_[~income_['ts_code'].str[0].isin(['A'])]
        cash_ = cash_[~cash_['ts_code'].str[0].isin(['A'])]

        income_ = income_[income_[['oper_cost', 'int_exp','comm_exp','sell_exp','admin_exp']].sum(1) > 0]
 

        # 会有漏的
        balance_, income_,cash_ = tl.add_fin(balance_, income_,cash_, q,pro.balancesheet,today, balance_col,'bs')
        income_, balance_,cash_ = tl.add_fin(income_, balance_,cash_, q,pro.income,today, income_col,'is')
        cash_, balance_, income_ = tl.add_fin(cash_, balance_, income_, q,pro.cashflow,today, cash_col,'cs')


        balance_ = balance_.sort_values(['update_flag'])
        balance_ = balance_.drop_duplicates(subset=['ts_code'], keep='first')
        balance_ = balance_.fillna(0)

        income_ = income_.sort_values(['update_flag'])
        income_ =  income_.drop_duplicates(subset=['ts_code'], keep='first')
        income_ = income_.fillna(0)

        cash_ = cash_.sort_values(['update_flag'])
        cash_ = cash_.drop_duplicates(subset=['ts_code'], keep='first')
        cash_ = cash_.fillna(0)


        # indicator_ = pro.fina_indicator_vip(period=q,fields=indi)
        balance_.to_pickle(fin_path_raw + 'raw' + '//' + q + '//' +  'balance' + '.pkl')
        income_.to_pickle(fin_path_raw + 'raw' + '//' + q + '//'+ 'income' + '.pkl')
        cash_.to_pickle(fin_path_raw + 'raw' + '//' + q + '//'+ 'cash' + '.pkl')
...
# df = pro.cashflow(ts_code='603288.SH', start_date='20180101', end_date='20251231')
# df = pro.cashflow(ts_code='603288.SH', start_date='20180101', end_date='20251231',fields = cash_col)
